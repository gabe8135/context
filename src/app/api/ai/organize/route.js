...
export async function POST(request) {
  ...
  try {
    ...
    const values = inputSchema.parse(body);
    const { supabase, workspaceId } = await requireWorkspace();
    let context = "Área geral do workspace";
    ...
    const items = (result.items || []).map((item, index) => ({ ...item, id: `ai-${index}`, selected: true }));
    return NextResponse.json({ items, summary: result.summary, mode: "ai" });
  } catch (error) {
    if (rawText.trim().length >= 3) return NextResponse.json({ items: parseUnstructuredText(rawText), summary: "A IA ficou indisponível; aplicamos o organizador local seguro.", mode: "fallback", warning: "A análise usou o modo local. Você pode revisar e importar normalmente." });
