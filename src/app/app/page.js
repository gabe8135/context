...
import { AlertTriangle, CalendarCheck, CheckSquare2, FolderKanban, Plus, StickyNote } from "lucide-react";
...
function FocusList({ icon: Icon, title, rows, empty, danger = false }) {
  return <section className={`focus-list ${danger ? "danger" : ""}`}><header><div><Icon size={18}/><h2>{title}</h2></div><span className="badge">{rows.length}</span></header><div>{rows.length ? rows.slice(0, 7).map((task) => <Link className="focus-task" href={`/app/tarefas/${task.id}`} key={task.id}><div><b>{task.title}</b><span>{task.projects?.name || "Pessoal"} · {task.priority}</span></div>{task.due_at && <time>{new Date(task.due_at).toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })}</time>}</Link>) : <p className="focus-empty">{empty}</p>}</div></section>;
}

function PersonalList({ icon: Icon, title, rows, type, empty }) {
  return <section className="personal-agenda-card"><header><div><Icon size={18}/><h3>{title}</h3></div><span className="badge">{rows.length}</span></header><div>{rows.length ? rows.slice(0, 5).map((item) => <Link className="personal-agenda-item" href={`/app/${type === "task" ? "tarefas" : "notas"}/${item.id}`} key={item.id}><b>{item.title}</b><span>{type === "task" ? (item.due_at ? `Prazo: ${new Date(item.due_at).toLocaleDateString("pt-BR")}` : "Sem prazo") : item.content.slice(0, 100)}</span></Link>) : <p className="focus-empty">{empty}</p>}</div><Link className="personal-list-link" href={`/app/${type === "task" ? "tarefas" : "notas"}`}>Ver tudo</Link></section>;
}
