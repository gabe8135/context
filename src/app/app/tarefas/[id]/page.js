...
export default async function TaskDetail({ params, searchParams }) {
  ...
  return <AppShell context={project ? { type: "project", ...project } : null}><div className="content narrow">
    ...
    <form action={updateTaskAction.bind(null, id)} className="panel form-panel"><div className="form-grid">
      ...
      <label className="field"><span>Prioridade</span><select name="priority" defaultValue={task.priority}>{["low", "medium", "high", "urgent"].map((value) => <option key={value}>{value}</option>)}</select></label>
