...
export default async function Tasks({ searchParams }) {
  ...
  let query = supabase.from("tasks").select("id,title,description,status,priority,starts_at,due_at,next_action,completed_at,created_at,queue_position,depends_on_task_id,projects(name,slug),clients(name)").eq("workspace_id", workspaceId).is("archived_at", null);
  ...
  const tasks = await attachTaskDependencies(supabase, data || []);
  ...
  return <AppShell context={scope ? { type: "project", ...scope } : null}><div className={`content tasks-list-page ${scope ? "scoped-list-page" : ""}`}>
    ...
    <section className="panel data-panel tasks-compact-list"><table><thead><tr>{scope && <th>Fila</th>}<th>Tarefa</th><th>Contexto</th><th>Status</th><th>Prazo</th><th>Ações</th></tr></thead><TaskListRows initialTasks={tasks} scope={scope}/></table>{!tasks.length && <div className="empty">Nenhuma tarefa neste contexto.</div>}</section>
  </div></AppShell>;
}
