...
export function ImportReview({ projects, action, initialProjectId = "" }) {
  ...
  return <div>
    <section className="panel form-panel">
      <label className="field"><span>Contexto da análise</span><select value={projectId} onChange={(event) => setProjectId(event.target.value)}><option value="">Área geral</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name} · {project.clients?.name}</option>)}</select><small className="meta">Selecionar o projeto ajuda a IA a interpretar nomes, tarefas e decisões corretamente.</small></label>
    ...
    {items.length > 0 && <form action={action} className="panel" style={{ marginTop: 24, padding: 19 }}>
      ...
      <label className="field"><span>Projeto de destino</span><select name="project_id" required value={projectId} onChange={(event) => setProjectId(event.target.value)}><option value="" disabled>Selecione</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name} · {project.clients?.name}</option>)}</select></label>
