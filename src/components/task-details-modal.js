...
export function TaskDetailsModal({ task, projectSlug, onClose }) {
  ...
  return createPortal(<div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="app-modal" role="dialog" aria-modal="true" aria-labelledby="task-modal-title">
      ...
      <div className="modal-grid"><Detail label="Status" value={task.status.replaceAll("_", " ")}/><Detail label="Prioridade" value={task.priority}/><Detail label="Início" value={formatDate(task.starts_at)}/><Detail label="Prazo" value={formatDate(task.due_at)}/><Detail label="Criada em" value={formatDate(task.created_at)}/><Detail label="Concluída em" value={formatDate(task.completed_at)}/></div>
